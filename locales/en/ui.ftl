...
settings-skills-hint = Installs a snippet that teaches the agent (Claude Code, Codex, Antigravity) how to use cadenza-cli. The snippet is written to the selected scope (current project or global).
...
task-worktree-create = Create worktree
task-worktree-switch = Switch branch
task-worktree-remove = Remove worktree
task-worktree-working = Running git…
task-worktree-created = Worktree created.
task-worktree-switched = Switched to branch { $branch }.
task-worktree-removed = Worktree removed.
task-worktree-fields-required = Enter the branch and the worktree path.
task-worktree-defaults-error = Could not read the current branch: { $error }
task-worktree-error = Git error: { $error }
confirm-remove-worktree = Remove this task's worktree? Git refuses if there are uncommitted changes.
