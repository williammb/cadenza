...
terminal-empty = Arraste uma aba aqui para abrir o terminal
...
task-tab-detalhes = Detalhes
task-tab-worktree = Worktree
terminal-open-aria = Mostrar ou ocultar o terminal
terminal-pane-close-aria = Remover este painel da grade
terminal-add-to-grid = Adicionar { $id } à grade
terminal-remove-from-grid = Remover { $id } da grade
terminal-tab-placed = Na grade
terminal-tab-unplaced = Fora da grade
terminal-add-zone = Soltar para adicionar painel
terminal-grid-full = Grade cheia (máximo de 4 painéis)
terminal-grid-status = { $id }: { $n } de 4 na grade
terminal-output-pending = saída não visualizada
...
start-agent-auto-mode-unavailable = Auto Mode não está disponível para este agente.
...
jira-assigned-filter-ph = Filtrar issues…
