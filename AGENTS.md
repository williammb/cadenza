...
- **XSS hygiene in the UI.** `innerHTML` is forbidden everywhere except `ui/markdown.js` (the markdown renderer, which sanitizes via DOMPurify before assignment). Use `textContent` + `createElement`/`append` for everything else. CI enforces this: `rg '\.innerHTML\s*=' ui/ --type js -g '!**/vendor/**' -g '!**/markdown.js'` — any match fails the build. `ui/vendor/**` and `ui/markdown.js` are the only entries in the allowlist.
